...
headerCells[3].addEventListener("click", ()=>{
    ...
    const sortedRows = rows.sort((rowA, rowB)=>{
        const ageA = parseInt(rowA.querySelector("td:nth-child(4)").textContent.trim(), 10);
        const ageB = parseInt(rowB.querySelector("td:nth-child(4)").textContent.trim(), 10);
...
headerCells[4].addEventListener("click", ()=>{
    ...
    const sortedRows = rows.sort((rowA, rowB)=>{
        const salA = parseInt(rowA.querySelector("td:nth-child(5)").textContent.trim().replace(/[$,]/g, ""), 10);
        const salB = parseInt(rowB.querySelector("td:nth-child(5)").textContent.trim().replace(/[$,]/g, ""), 10);
...
form.addEventListener("submit", function(e) {
    ...
    const formPosition = form.querySelector('input[name="position"]').value.trim();
    const formAge = parseInt(form.querySelector('input[name="age"]').value.trim(), 10);
    const formSalary = parseInt(form.querySelector('input[name="salary"]').value.trim().replace(/[$,]/g, ""), 10);
    const formOffice = form.querySelector('select[name="office"]').value;
    // Validate all fields are filled
    if (!formName || !formPosition || !formOffice || isNaN(formAge) || isNaN(formSalary)) {
